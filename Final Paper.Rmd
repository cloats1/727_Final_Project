...
combo_pg_census <- census_pg %>% 
  ...
  select(prop_white, prop_black, prop_latino) %>%
  gather(key="Race", value= "population")
...
combo_moco_census <-census_moco %>%
  ...
  select(prop_white, prop_black, prop_latino) %>%
  gather(key="Race", value= "population") 
census_race_1 <- ggplot() + geom_bar(data=combo_pg_census, aes(Race, population),fill="blue", stat="identity")
census_race_1
census_race_2 <- ggplot() + geom_bar(data=combo_moco_census, aes(Race, population),fill="black", stat="identity")
census_race_2
```


The Census ACS 2020 data provides data on racial makeup of each of our counties. For this study we looked at White, Black and Latino. A chi-squared test was performed for each pair or proportions to test equality. All three were statistically significantly not equal but the Latino difference was much less extreme. The proportions there were approximately 18.8% for Prince George's County and 19.5% for Montgomery County. On the other hand the proportion of Prince George's County that is Black is approximately 62.2% and for Montgomery County it is only 18.5%. Such a difference could very well be an explanation for why COVID-19 impacts Prince George's County differently than Montgomery County.
...
```{r}
census_race <- ggplot() + geom_bar(data=census_pg_prop, aes(prop_white),color="blue") + geom_bar(data=census_pg_prop, aes(prop_black),color="blue") + geom_bar(data=census_pg_prop, aes(prop_latino),color="blue") + geom_bar(data=census_moco_prop, aes(prop_white),color="black") + geom_bar(data=census_moco_prop, aes(prop_black),color="black") + geom_bar(data=census_moco_prop, aes(prop_latino),color="black") +coord_flip()
census_race
```
...
prop_pg <- pg_jhu_prop %>%
  group_by(time_value, value)
prop_moco <- moco_jhu_prop %>%
  group_by(time_value, value)
prop <- ggplot()+geom_line(data=prop_pg, aes(time_value,value), color="blue") + 
  geom_line(data=prop_moco, aes(time_value,value), color="black") + labs(title= "Cumulative number of confirmed deaths due to COVID-19,\n per 100,000 population")
prop
```
